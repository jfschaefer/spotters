...
for row in endpoint.query(f'''
SELECT DISTINCT ?paper WHERE {{
# GRAPH <http://localhost:8890/asa_id_grounding_data> {{ }}
    ?anno {OA.hasTarget:<>}/{OA.hasSource:<>} ?paper .
    ?anno {OA.hasBody:<>} ?body .
    ?body <https://sigmathling.kwarc.info/resources/grounding-dataset/hasSog> ?sog .
}} LIMIT 100
'''):
    print(row)
